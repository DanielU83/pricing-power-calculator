...
export default function Page() {
  ...
  const structureData = [
    ...
    {
      name: "Profit",
      value: Math.max(0, (1 - cogsPct / 100 - fixedPct / 100) * 100),
      fill: structure.isValid ? colors.green : colors.slate
    }
  ];
  ...
  const improvementData = [
    ...
    {
      lever: "Volume",
      improvement: onePercentImpacts.volume,
      fill: colors.purple
    },
    {
      lever: "Variable Costs",
      improvement: onePercentImpacts.cogs,
      fill: colors.red
    },
    {
      lever: "Fixed Costs",
      improvement: onePercentImpacts.fixed,
      fill: colors.orange
    }
  ];
  ...
  const tradeoffData = [
    {
      label: "Price Change",
      value: priceChangeTradeoffPct / 100,
      fill: colors.blue
    },
        ...
        <section className="grid gap-6 md:grid-cols-3 rounded-2xl bg-white p-6 shadow-sm border border-slate-200">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-[1fr_auto_auto] gap-y-3 gap-x-3 items-center text-sm">
              ...
              <span className="justify-self-end font-mono font-bold text-slate-900">
                {salesPct.toFixed(1)}
              </span>
              ...
              <input
                ...
                value={cogsPct}
                onChange={(e) =>
                  setCogsPct(clamp(Number(e.target.value || 0), 0, 100))
                }
              />
              <span className="text-xs text-slate-400 w-8">%</span>
              ...
              <input
                ...
                value={fixedPct}
                onChange={(e) =>
                  setFixedPct(clamp(Number(e.target.value || 0), 0, 100))
                }
              />
              <span className="text-xs text-slate-400 w-8">%</span>
              ...
              <span
                className={`justify-self-end font-mono font-bold ${
          ...
          <div className="flex flex-col justify-between">
            <div>
              ...
              <p className="text-sm text-slate-600 leading-relaxed">
                If you raise price by{" "}
                <span className="font-bold text-blue-600 bg-blue-50 px-1 rounded">
                  1%
                </span>{" "}
                (holding volume constant), your operating profit grows by:
              </p>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    onChange={(e) =>
                      setPriceChangeTradeoffPct(Number(e.target.value))
                    }
                  ...
                  <div className="relative w-20">
                    <input
                      ...
                      value={priceChangeTradeoffPct}
                      onChange={(e) =>
                        setPriceChangeTradeoffPct(
                          clamp(Number(e.target.value || 0), -50, 50)
                        )
                      }
                    />
              ...
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                ...
                <p
                  className={`text-2xl font-bold ${
                    tradeoff.volumeRequired > 0
                      ? "text-green-600"
                      : "text-slate-700"
                  }`}
                >
                  {invalid || !Number.isFinite(tradeoff.volumeRequired)
                    ? "-"
                    : (tradeoff.volumeRequired > 0 ? "+" : "") +
                      formatPercent(tradeoff.volumeRequired, 1)}
                </p>
                ...
                >
                  ...
                  <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                    {tradeoffData.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={entry.value < 0 ? colors.red : entry.fill}
                      />
                    ))}
                    <LabelList
                      dataKey="value"
